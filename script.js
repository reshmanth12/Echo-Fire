...
// === GLOBAL KEY HANDLER (STRICT MODE) ===
window.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter') {
        ...
        if (gameState === 'ROLL') {
            ...
            return; // STRICT RETURN: Don't click buttons
        }
        ...
        // --- MENU INPUTS ---
        if (gameState === 'SETUP' || gameState === 'PAUSE_MENU' || gameState === 'VICTORY') {
            if (currentFocusEl) {
...
function togglePauseMenu() {
    ...
    if (gameState === 'PAUSE_MENU') {
        ...
        gameState = (currentRollValue !== null) ? 'SHOOT' : 'ROLL';
...
function handleShootingNav(key) {
    ...
    let idx = shootingTargets.findIndex(t => `cell-${t.playerId}-${t.rowNum}` === (currentFocusEl ? currentFocusEl.id : ''));
    if(idx === -1) idx = 0;
    if (key === 'ArrowRight' || key === 'ArrowDown') { idx = (idx + 1) % shootingTargets.length; } else { idx = (idx - 1 + shootingTargets.length) % shootingTargets.length; }
    const t = shootingTargets[idx];
    const el = document.getElementById(`cell-${t.playerId}-${t.rowNum}`);
    setFocus(el);
}

function handleShootingFire() {
    if (!currentFocusEl || !currentFocusEl.classList.contains('valid-target')) return;
    const parts = currentFocusEl.id.split('-');
    shoot(players[currentPlayerIndex].id, parseInt(parts[1]), parseInt(parts[2]));
}
...
function executeBotShot(botId) {
    ...
    setTimeout(() => {
        ...
        targetEl.classList.add('valid-target'); targetEl.classList.add('keyboard-focus');
        setStatus(`Bot Locked: ${players[finalTarget.id].name} (Stage ${finalTarget.stage})`);
        setTimeout(() => { targetEl.classList.remove('valid-target'); targetEl.classList.remove('keyboard-focus'); shoot(botId, finalTarget.id, finalTarget.row); }, 1000);
    }, 1500);
}
...
function shoot(sId, vId, vNum) {
    ...
    document.querySelectorAll('.keyboard-focus').forEach(e => e.classList.remove('keyboard-focus'));
